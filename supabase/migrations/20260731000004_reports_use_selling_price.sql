-- Reports must always reflect the selling price (what the customer pays in SYP),
-- never the transfer quantity (the SIM balance being moved).
--
-- New transfers now persist `package_price` via device-sync. Existing rows keep
-- their quantity in `amount` and `package_price` is NULL, so the report function
-- falls back to `amount` for legacy rows via COALESCE.
ALTER TABLE IF EXISTS public.transfers
  ADD COLUMN IF NOT EXISTS package_price NUMERIC;

-- Backfill: nothing safe to backfill without the preset mapping on the server,
-- so legacy rows report their quantity as a graceful fallback (handled below).
