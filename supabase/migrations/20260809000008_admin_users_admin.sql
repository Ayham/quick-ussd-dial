-- =============================================================================
-- admin_get_users_admin: enhanced admin users listing.
--
-- Returns, for every user:
--   * all profile + auth info (same fields as admin_get_all_users_license)
--   * role (real source: user_roles; NOT profiles.role which holds legacy
--     distributor/customer values)
--   * payments_summary : totals grouped per currency (never mixed) + count
--   * notifications_summary : delivered/failed/pending + total + unread
--   * activations_summary : counts per status + latest status + latest date
--
-- Extra filters: _account_status, _role ('admin'/'user'), _activation_status.
